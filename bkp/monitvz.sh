...
do
	...
	CPU=`zabbix_get -s 200.129.21.21 -k "system.cpu.util[,,avg1]"`

	BUFFERS=`zabbix_get -s 200.129.21.21 -k "vm.memory.size[buffers]"`
	CACHED=`zabbix_get -s 200.129.21.21 -k "vm.memory.size[cached]"`
	USED=`zabbix_get -s 200.129.21.21 -k "vm.memory.size[used]"`
	MEM_USED=`expr $USED - $BUFFERS - $CACHED`
	MEM_USED=`expr $MEM_USED / 1024 / 1024`


	NET_IN=`zabbix_get -s 200.129.21.21 -k "net.if.in[venet0,bytes]"`
	NET_OUT=`zabbix_get -s 200.129.21.21 -k "net.if.out[venet0,bytes]"`
